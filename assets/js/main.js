...
(function () {
        ...
        for (let phi = 0; phi < 6.28; phi += 0.02) {
          ...
          if (yp >= 0 && yp < numRows && xp >= 0 && xp < numCols) {
            if (ooz > zbuffer[xp][yp]) {
              ...
              const charIdx = Math.floor(luminanceIdx * depthFactor + (luminanceChars.length - 1) * (1 - depthFactor) * 0.3);
              ...
              if (glyphMap[key]) {
                ...
                output[xp][yp] = luminanceChars[Math.max(0, Math.min(charIdx, luminanceChars.length - 1))];
              }
            }
          }
        }
      ...
      for (let x = 0; x < numCols; x++) {
        for (let y = 0; y < numRows; y++) {
          ...
          if (char && char !== ' ') {
            ...
            const baseR = 91, baseG = 143, baseB = 185;
  ...
  function renderFeaturedProject() {
    ...
    if (project.status) {
      ...
      status.appendChild(createElement('span', { textContent: project.status + (project.statusDetails ? ' — ' + project.statusDetails : '') }));
    ...
    if (project.featuredQuote) {
      content.appendChild(createElement('blockquote', { className: 'featured-project__quote', textContent: project.featuredQuote }));
    }

    // Description
    content.appendChild(createElement('p', { className: 'featured-project__description', textContent: project.longDescription || project.shortDescription }));

    // Architecture notes
    if (project.architectureNotes) {
      const archSection = createElement('div', { className: 'featured-project__architecture' });
      archSection.appendChild(createElement('div', { className: 'featured-project__architecture-title' }, ['ARCHITECTURE']));
      archSection.appendChild(createElement('p', { className: 'featured-project__architecture-text', textContent: project.architectureNotes }));
    ...
    if (project.technicalHighlights && project.technicalHighlights.length) {
      ...
      hlSection.appendChild(createElement('div', { className: 'featured-project__highlights-title' }, ['HIGHLIGHTS']));
      project.technicalHighlights.forEach(hl => {
        hlSection.appendChild(createElement('div', { className: 'featured-project__highlight-item', textContent: hl }));
      });
  ...
  function createProjectCard(project, variant = 'standard') {
    ...
    if (variant === 'featured') {
      ...
      if (project.status) meta.appendChild(createElement('span', { className: 'project-card__status', textContent: project.status }));
      content.appendChild(meta);
      content.appendChild(createElement('h3', { className: 'project-card__title', textContent: project.title }));
      content.appendChild(createElement('p', { className: 'project-card__description', textContent: project.shortDescription }));
      if (project.tech && project.tech.length) {
  ...
  function createBlenderCard(item) {
    ...
    meta.appendChild(document.createTextNode(formatDate(item.date) + (item.renderEngine ? ' · ' + item.renderEngine : '')));
    content.appendChild(meta);

    if (item.description) {
      content.appendChild(createElement('p', { className: 'blender-card__description', textContent: item.description }));
    }

    if (item.techniques && item.techniques.length) {
      const techniques = createElement('div', { className: 'blender-card__techniques' });
      item.techniques.forEach(t => techniques.appendChild(createElement('span', { className: 'blender-card__technique' }, [t])));
  ...
  function renderContact() {
    ...
    if (linksContainer) {
      ...
      if (contact.email) {
        linksContainer.appendChild(createElement('a', {
  ...
  function showAdminPasswordModal() {
    ...
    modal.appendChild(createElement('h3', { style: 'font-family: "JetBrains Mono", monospace; font-size: 0.85rem; color: var(--accent-blue); margin-bottom: 1rem; letter-spacing: 0.1em;', textContent: 'ADMIN ACCESS' }));
  ...
  function renderNavbar() {
    ...
    if (navLinks) {
      ...
      SITE_DATA.nav.forEach(item => {
        ...
        // Set active
        if (currentPath.endsWith(item.href) || (item.href === '/index.html' && (currentPath === '/' || currentPath.endsWith('/')))) {
          link.classList.add('active');
        }
        li.appendChild(link);
  ...
  function renderFooter() {
    ...
    if (copyright) {
      ...
      copyright.appendChild(document.createTextNode(' ' + SITE_DATA.footer.year + ' ' + SITE_DATA.footer.copyright));
    }
