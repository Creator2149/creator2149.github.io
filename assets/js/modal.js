...
(function () {
  ...
  function openProjectModal(project) {
    ...
    if (project.year) html += `<span style="font-family:var(--font-mono);font-size:0.75rem;color:var(--text-tertiary);">${escapeHTML(project.year)}</span>`;
    if (project.status) html += `<span style="font-family:var(--font-mono);font-size:0.75rem;color:var(--accent-amber);">${escapeHTML(project.status)}</span>`;
    html += '</div>';
    html += '</div>';
    ...
    if (project.image && project.image.trim()) {
      html += `<div style="margin-bottom:1.5rem;aspect-ratio:16/9;background:var(--bg-tertiary);border:1px solid var(--border);overflow:hidden;">
        <img src="${escapeHTML(project.image)}" alt="${escapeHTML(project.title)}" style="width:100%;height:100%;object-fit:cover;" loading="lazy" onerror="this.parentElement.innerHTML='<div class=no-image>NO PREVIEW</div>'">
      </div>`;
    }
  ...
  function openCertificateModal(cert) {
    ...
    html += '<div style="display:flex;gap:1rem;align-items:center;margin-top:0.5rem;">';
    if (cert.year) html += `<span style="font-family:var(--font-mono);font-size:0.75rem;color:var(--text-tertiary);">${escapeHTML(String(cert.year))}</span>`;
    if (cert.field) html += `<span style="font-family:var(--font-mono);font-size:0.75rem;color:var(--accent-blue);">${escapeHTML(cert.field)}</span>`;
    html += '</div>';
    html += '</div>';
  ...
  function openBlenderModal(item) {
    ...
    html += '<div style="display:flex;gap:1rem;align-items:center;margin-top:0.5rem;">';
    if (item.date) html += `<span style="font-family:var(--font-mono);font-size:0.75rem;color:var(--text-tertiary);">${escapeHTML(item.date)}</span>`;
    if (item.renderEngine) html += `<span style="font-family:var(--font-mono);font-size:0.75rem;color:var(--accent-amber);">${escapeHTML(item.renderEngine)}</span>`;
    html += '</div>';
    html += '</div>';
